...
| 22 | Flux has **no** `smart_format` | VERIFIED (by absence + structure) | The documented Flux `Configure` schema exposes only `thresholds`, `keyterms`, `language_hints`; `smart_format` appears only on `/v1/listen`. Numerals doc treats numerals as the Flux formatting story. |
